...
if (qte_active && keyboard_check_pressed(vk_space)) {
    ...
    if (instance_exists(arrow_instance)) {
        ...
        if (arrow_instance.x >= target_x && arrow_instance.x <= target_x + target_width) {
			...
			randomize();
			gunDamage = floor(random_range(obj_battle_interface.fireL, obj_battle_interface.fireH + 1));
			
			
            obj_battle_interface.enemHealth -= gunDamage;
            obj_battle_interface.meleeDamage = gunDamage;
